...
def run_consumer(server=None, port=None):
    ...
    daily_filepath = f"{path_to_out_dir}/SOC_WW_residues_6.csv"
    with open(daily_filepath, "wt", newline="", encoding="utf-8") as daily_f:
        ...
        aomf_header_row = [f"AOMf_{i}" for i in range(1, 3)]
        daily_writer.writerow([
            "Exp", "Crop", "Date", "ZDPH", "NFert", "Irrig", "PYield", "AbBiom", "LAI", "TRANS", "ETa", "Roff", "DPER",
            "NLEA", "Nresid", "AOMs", "SMBf", "SMBs", "SOMf", "SOMs", "SOC_top20", "SOC_20to40", "SOC_40to60", "SOC_total"
        ] + swc_header_row + n_header_row + aomf_header_row)
            ...
            try:
                ...
                for vals in results:
                    ...
                    row = [exp_no, vals["Crop"], vals["Date"], vals["Stage"], vals["NFert"], vals["Irrig"],
                           ...
                           vals["DPER"], vals["NLEA"], vals["Nresid"], vals["AOMs"], vals["SMBf"], 
                           vals["SMBs"], vals["SOMf"], vals["SOMs"], round(vals["SOC_top20"], 3), round(vals["SOC_20to40"], 3), 
                           round(vals["SOC_40to60"], 3), round(vals["SOC_total"], 3)] + swc_data + n_data + aomf_data
